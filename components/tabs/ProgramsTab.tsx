...
const ProgramReportModal: React.FC<{program: FertilizationProgram; onClose: () => void;}> = ({ program, onClose }) => {
    ...
    if (!reportData) return <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4"><div className="bg-white p-4 rounded-lg">خطأ.</div></div>;
    const { expenses, revenues, totalExpenses, totalRevenue, farmerShare, ownerProfit, cycle } = reportData;

    return <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" onClick={onClose}><div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col" onClick={e=>e.stopPropagation()}><div className="flex justify-between items-center mb-4 pb-4 border-b"><h2 className="text-2xl font-bold">تقرير: {program.name}</h2><button onClick={onClose}><CloseIcon className="w-6 h-6" /></button></div><div className="flex-grow overflow-y-auto space-y-6"><div className="grid grid-cols-1 md:grid-cols-3 gap-4"><ReportStatCard title="إجمالي مصروفات البرنامج" value={formatCurrency(totalExpenses)} icon={<ExpenseIcon className="w-7 h-7 text-rose-500" />} /><ReportStatCard title="إيرادات البرنامج" value={formatCurrency(totalRevenue)} icon={<RevenueIcon className="w-7 h-7 text-emerald-500" />} /><ReportStatCard title="صافي ربح المالك" value={formatCurrency(ownerProfit)} icon={<ProfitIcon className={`w-7 h-7 ${ownerProfit >= 0 ? 'text-sky-500' : 'text-orange-500'}`} />} /></div>{settings.isFarmerSystemEnabled && cycle.farmerId && <ReportStatCard title={`حصة المزارع (${cycle.farmerSharePercentage}%)`} value={formatCurrency(farmerShare)} icon={<FarmerIcon className="w-7 h-7 text-indigo-500" />} />}<div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><div><h3 className="text-lg font-semibold mb-2">المصروفات ({expenses.length})</h3><div className="border rounded-lg max-h-60 overflow-y-auto">{expenses.length > 0 ? expenses.map(t=><div key={t.id} className="p-2 border-b flex justify-between"><span>{t.description}</span><strong>{formatCurrency(t.amount)}</strong></div>) : <p>لا يوجد.</p>}</div></div><div><h3 className="text-lg font-semibold mb-2">الإيرادات ({revenues.length})</h3><div className="border rounded-lg max-h-60 overflow-y-auto">{revenues.length > 0 ? revenues.map(t=><div key={t.id} className="p-2 border-b flex justify-between"><span>{t.description}</span><strong>{formatCurrency(t.amount)}</strong></div>) : <p>لا يوجد.</p>}</div></div></div></div></div></div>
};
...
const ProgramsTab: React.FC<{ cycle: CropCycle }> = ({ cycle }) => {
    ...
    const handleSave = (program: Omit<FertilizationProgram, 'id'> | FertilizationProgram) => {
        if ('id' in program) updateFertilizationProgram(program);
        else addFertilizationProgram(program);
        setModal(null);
    };

    const confirmDelete = () => {
        if (deletingId) deleteFertilizationProgram(deletingId);
        setDeletingId(null);
    };
