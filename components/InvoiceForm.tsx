...
const InvoiceForm: React.FC<{
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                <label htmlFor="description" className="block text-sm font-medium text-gray-700 dark:text-gray-300">وصف الفاتورة</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="date" className="block text-sm font-medium text-gray-700 dark:text-gray-300">تاريخ الفاتورة</label>
            ...
            <div className="space-y-3">
                {items.map((item, index) => (
                    <fieldset key={index} className="border border-gray-300 dark:border-gray-600 p-4 rounded-md relative group">
                        <legend className="px-2 text-sm font-medium text-gray-700 dark:text-gray-300">سعر {index + 1}</legend>
                        {items.length > 1 && (
                            <button
                                type="button"
                                onClick={() => removeItem(index)}
                                className="absolute top-2 left-2 p-1 text-gray-400 hover:text-red-500 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 opacity-0 group-hover:opacity-100 transition-opacity"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label htmlFor={`quantity-${index}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300">الكمية (ك.ج)</label>
                             ...
                             <div>
                                <label htmlFor={`price-${index}`} className="block text-sm font-medium text-gray-700 dark:text-gray-300">سعر الكيلو</label>
            ...
            <div className="mt-4">
                <button
                    type="button"
                    onClick={addItem}
                    className="flex items-center justify-center w-full px-4 py-2 border-2 border-dashed border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700/50 hover:border-green-500 hover:text-green-600 dark:hover:text-green-400 transition-colors"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                 <div>
                    <label htmlFor="discount" className="block text-sm font-medium text-gray-700 dark:text-gray-300">الخصم (ج.م)</label>
                 ...
                 <div className="pt-1">
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">الإجمالي</p>
