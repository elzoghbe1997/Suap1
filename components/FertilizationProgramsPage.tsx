...
const ProgramForm: React.FC<{
    ...
    onCancel: () => void;
    cropCycles: CropCycle[]; // Expects pre-filtered active cycles
    ...
    const selectableCycles = React.useMemo(() => {
        // The cropCycles prop is pre-filtered by the parent component to only include active cycles.
        return [...cropCycles].sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime());
    }, [cropCycles]);
    
    React.useEffect(() => {
        if (!program && selectableCycles.length === 1) {
            setCropCycleId(selectableCycles[0].id);
        }
    }, [program, selectableCycles]);
...
const FertilizationProgramsPage: React.FC = () => {
    ...
    const handleSave = React.useCallback((program: Omit<FertilizationProgram, 'id'> | FertilizationProgram) => {
        if ('id' in program) {
        ...
        setModal(null);
    }, [addFertilizationProgram, updateFertilizationProgram]);
    ...
    const confirmDelete = React.useCallback(() => {
        ...
        setDeletingId(null);
    }, [deletingId, deleteFertilizationProgram]);
    
    const cyclesWithPrograms = React.useMemo(() => {
        return cropCycles
    ...
    const renderCyclePrograms = (cycle: (typeof cyclesWithPrograms)[0]) => {
    ...
    const renderContent = () => {
        ...
        if (cyclesWithPrograms.length === 0) return (
            <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-lg border-2 border-dashed border-slate-200 dark:border-slate-700">
                ...
                <p className="text-lg font-semibold text-slate-600 dark:text-slate-300">لا توجد برامج مسجلة للعروات النشطة</p>
        ...
        return (
            <div className="space-y-8">
                {cyclesWithPrograms.map(renderCyclePrograms)}
            </div>
        );
    };
        ...
        <div className="space-y-6">
            ...
            {(modal === 'ADD' || modal === 'EDIT') && (
                <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" onClick={() => setModal(null)}>
                    <div className="bg-slate-50 dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                        <div className="p-6 pb-4 border-b border-slate-200 dark:border-slate-700 flex-shrink-0">
                            <h2 className="text-2xl font-bold text-slate-800 dark:text-white">
                                {modal === 'EDIT' ? 'تعديل البرنامج' : 'إضافة برنامج جديد'}
                            </h2>
                        </div>
                        <div className="p-6 flex-grow overflow-y-auto modal-scroll-contain">
                            <ProgramForm
                                ...
                                cropCycles={cropCycles.filter(c => c.status === CropCycleStatus.ACTIVE)}
                            />
                        </div>
                    </div>
                </div>
